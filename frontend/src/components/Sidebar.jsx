        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all duration-150 group ${active
                  ? "nav-active text-text-primary"
                  : "text-text-muted hover:text-text-primary hover:bg-bg-card"
                }`}
            >
              <span
                className={`material-symbols-outlined mr-3 text-[18px] transition-colors ${active ? "text-accent-green" : "group-hover:text-text-secondary"
                  }`}
        ...
        <div className="bg-bg-card border border-border-dark rounded-xl p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="flex items-center gap-1.5">
              <span
                className={`w-1.5 h-1.5 rounded-full ${systemStatus === "live" ? "bg-accent-green animate-pulse" : "bg-red-500 animate-pulse"
                  }`}
              />
              <span
                className={`text-[10px] font-semibold ${systemStatus === "live" ? "text-accent-green" : "text-red-500"
                  }`}
              >
          ...
          <div className="w-full bg-border-dark h-1 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${systemStatus === "live" ? "" : "w-0"
                }`}
              style={{
